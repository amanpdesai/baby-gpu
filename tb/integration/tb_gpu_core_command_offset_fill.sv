...
module tb_gpu_core_command_offset_fill;
  ...
  localparam int GRID_Y = 3;
  localparam int RECT_X = 1;
  localparam int RECT_Y = 1;
  localparam logic [15:0] FILL_COLOR = 16'h0A5C;
  localparam logic [15:0] SENTINEL = 16'hDEAD;
  ...
  function automatic logic [15:0] expected_pixel(input int px, input int py);
    begin
      expected_pixel = (px == RECT_X && py == RECT_Y) ? FILL_COLOR : SENTINEL;
    end
  endfunction
  ...
  function automatic logic [15:0] pixel_at(input int px, input int py);
    ...
    logic [31:0] word;
    begin
      pixel_idx = (py * GRID_X) + px;
      word = read_memory_word(FRAMEBUFFER_BASE + 32'(pixel_idx * 2));
      pixel_at = (pixel_idx % 2) == 0 ? word[15:0] : word[31:16];
    end
  endfunction
  ...
  initial begin
    ...
    init_memory({SENTINEL, SENTINEL});

    step();
    reset = 1'b0;
    step();

    load_offset_fill_program();
    ...
    wait_idle(500, "command-driven offset fill timed out");

    check(error_status == 8'h00, "command-driven offset fill has no errors");
    check(saw_mem_req, "command-driven offset fill reaches memory path");

    for (y = 0; y < GRID_Y; y = y + 1) begin
      for (x = 0; x < GRID_X; x = x + 1) begin
        check(pixel_at(x, y) == expected_pixel(x, y),
              "command-driven offset fill pixel matches");
      end
    end

    check(!busy, "gpu_core is idle after command-driven offset fill");

    $display("tb_gpu_core_command_offset_fill PASS");
    $finish;
  end
endmodule
