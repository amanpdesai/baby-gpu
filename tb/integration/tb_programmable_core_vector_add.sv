...
module tb_programmable_core_vector_add;
  ...
  localparam logic [ADDR_W-1:0] A_BASE = 32'h0000_0000;
  localparam logic [ADDR_W-1:0] B_BASE = 32'h0000_0040;
  localparam logic [ADDR_W-1:0] C_BASE = 32'h0000_0080;
  ...
  task automatic wait_kernel_done();
    ...
    begin
      ...
      while (!done && !error) begin
        ...
        check(cycles < 1000, "vector_add timeout");
        cycles = cycles + 1;
        step();
      end
      check(done && !error, "vector_add completes without core error");
      step();
    end
  endtask
  ...
  task automatic load_vector_add_program();
    begin
      ...
      write_imem(8'd5, isa_pkg::isa_r_type(ISA_OP_LOAD, 4'd5, 4'd4, 4'd0));
      ...
      write_imem(8'd8, isa_pkg::isa_r_type(ISA_OP_LOAD, 4'd6, 4'd4, 4'd0));
      ...
      write_imem(8'd12, isa_pkg::isa_r_type(ISA_OP_STORE, 4'd0, 4'd4, 4'd7));
  ...
  initial begin
    ...
    framebuffer_base = 32'h0000_0200;
    ...
    for (int idx = 0; idx < ELEMENTS; idx++) begin
      mem_write_word(A_BASE + ADDR_W'(idx * 4), 32'd100 + 32'(idx));
      mem_write_word(B_BASE + ADDR_W'(idx * 4), 32'd1000 + 32'(idx * 3));
    end
    mem_write_word(C_BASE + ADDR_W'(ELEMENTS * 4), 32'hCAFE_0006);
    mem_write_word(C_BASE + ADDR_W'((ELEMENTS + 1) * 4), 32'hCAFE_0007);

    launch_kernel(COORD_W'(ELEMENTS), 16'd1);
    wait_kernel_done();

    for (int idx = 0; idx < ELEMENTS; idx++) begin
      mem_read_expect(C_BASE + ADDR_W'(idx * 4), 32'd1100 + 32'(idx * 4));
    end

    mem_read_expect(C_BASE + ADDR_W'(ELEMENTS * 4), 32'hCAFE_0006);
    mem_read_expect(C_BASE + ADDR_W'((ELEMENTS + 1) * 4), 32'hCAFE_0007);

    check(!busy, "programmable core returns idle after vector_add");
    check(launch_ready, "programmable core can accept another launch after vector_add");
    check(!error, "programmable core leaves error low after vector_add");
    check(!mem_error, "data memory leaves error low after vector_add");

    $display("tb_programmable_core_vector_add PASS");
    $finish;
  end
endmodule
