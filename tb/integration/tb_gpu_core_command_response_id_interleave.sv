module tb_gpu_core_command_response_id_interleave;
  import isa_pkg::*;
  ...
  initial begin
    ...
    repeat (6) begin
      step();
      check(accepted_programmable == 0, "full writer response window accepts no programmable request");
    end

    release_response(2'b00);
    check(writer_pending == 3, "one writer response drained to open a programmable slot");
    ...
    check(saw_programmable_response_before_writer_drain, "programmable response drains while older writer responses remain pending");
    check(writer_pending == 3, "older writer responses still pending after programmable response");
    check(pixel_at(0) == KERNEL_COLOR, "programmable STORE16 write observed before older writer responses drain");
    check(error_status == 8'h00, "interleaved programmable response leaves sticky errors clear");
    ...
    while (busy || programmable_pending != 0) begin
      ...
      completion_timeout++;
      check(completion_timeout < 200, "response-ID interleave completion timeout");
    end

    check(saw_programmable_response_before_writer_drain, "programmable response returned before older writer responses");
    check(error_status == 8'h00, "response-ID interleave test leaves error clear");
    ...
    $display("tb_gpu_core_command_response_id_interleave PASS");
    $finish;
  end
endmodule
