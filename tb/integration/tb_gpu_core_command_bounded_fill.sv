...
module tb_gpu_core_command_bounded_fill;
  ...
  function automatic logic [15:0] expected_pixel(input int px, input int py);
    begin
      expected_pixel = (px < FILL_WIDTH && py < FILL_HEIGHT) ? FILL_COLOR : SENTINEL;
    end
  endfunction
  ...
  initial begin
    ...
    wait_idle(500, "command-driven bounded fill timed out");

    check(error_status == 8'h00, "command-driven bounded fill has no errors");
    check(saw_mem_req, "command-driven bounded fill reaches memory path");

    for (y = 0; y < GRID_Y; y = y + 1) begin
      for (x = 0; x < GRID_X; x = x + 1) begin
        check(pixel_at(x, y) == expected_pixel(x, y),
              "command-driven bounded fill pixel matches");
      end
    end

    check(!busy, "gpu_core is idle after command-driven bounded fill");

    $display("tb_gpu_core_command_bounded_fill PASS");
    $finish;
  end
endmodule
