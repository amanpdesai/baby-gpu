...
module tb_programmable_core_store16_framebuffer;
  ...
  task automatic wait_kernel_done();
    ...
    begin
      ...
      while (!done && !error) begin
        ...
        check(cycles < 1000, "store16 framebuffer timeout");
        cycles = cycles + 1;
        step();
      end
      check(done && !error, "store16 framebuffer kernel completes without core error");
      step();
    end
  endtask

  task automatic load_store16_program();
    begin
      ...
      write_imem(8'd3, kgpu_movsr(4'd3, ISA_SR_FRAMEBUFFER_BASE));
      write_imem(8'd4, kgpu_add(4'd4, 4'd3, 4'd1));
      write_imem(8'd5, kgpu_movsr(4'd5, ISA_SR_LINEAR_GLOBAL_ID));
      write_imem(8'd6, kgpu_movi(4'd6, 18'h50));
      write_imem(8'd7, kgpu_add(4'd5, 4'd5, 4'd6));
      write_imem(8'd8, kgpu_store16(4'd5, 4'd4, 18'd0));
  ...
  initial begin
    ...
    launch_kernel(COORD_W'(PIXELS), 16'd1);
    wait_kernel_done();

    mem_read_expect(FRAMEBUFFER_BASE + 32'd0, 32'h0051_0050);
    mem_read_expect(FRAMEBUFFER_BASE + 32'd4, 32'h0053_0052);
    mem_read_expect(FRAMEBUFFER_BASE + 32'd8, 32'hCCCC_0054);
    mem_read_expect(FRAMEBUFFER_BASE + 32'd12, 32'hDDDD_DDDD);

    check(!busy, "programmable core returns idle after store16 framebuffer kernel");
    check(launch_ready, "programmable core can accept another launch after store16 framebuffer kernel");
    check(!error, "programmable core leaves error low after store16 framebuffer kernel");
    check(!mem_error, "data memory leaves error low after store16 framebuffer kernel");

    $display("tb_programmable_core_store16_framebuffer PASS");
    $finish;
  end
endmodule
