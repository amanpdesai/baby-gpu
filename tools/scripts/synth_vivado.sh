...
sources=(
  ...
  "${repo_root}/platform/sim/data_memory.sv"
  "${repo_root}/platform/sim/gpu_video_controller_system.sv"
  "${repo_root}/platform/fpga/gpu_video_fpga_top.sv"
  "${repo_root}/rtl/core/command_fifo.sv"
  "${repo_root}/rtl/core/command_processor.sv"
  "${repo_root}/rtl/core/framebuffer_scanout.sv"
  "${repo_root}/rtl/core/framebuffer_swap_controller.sv"
  ...
  "${repo_root}/rtl/core/memory_response_tracker.sv"
  ...
  "${repo_root}/rtl/core/video_test_pattern.sv"
...
if [[ ! -f "${tcl_script}" ]]; then
  echo "Missing Vivado Tcl script: ${tcl_script}"
  exit 2
fi

for source in "${sources[@]}"; do
  if [[ ! -f "${source}" ]]; then
    echo "Missing Vivado source: ${source}"
    exit 2
  fi
done

if [[ "${dry_run}" == "1" ]]; then
  echo "VIVADO DRY-RUN ${top} (${VIVADO_PART})"
  echo "Vivado Tcl: ${tcl_script}"
...
vivado -mode batch -nojournal -nolog \
  -source "${tcl_script}" \
  -tclargs "${top}" "${VIVADO_PART}" "${out_path}" "${sources[@]}"
