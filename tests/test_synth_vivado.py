...
def test_vivado_dry_run_accepts_fpga_video_top(tmp_path):
    out_dir = tmp_path / "vivado_out"
    env = os.environ.copy()
    env.update(
        {
    ...
    assert "VIVADO DRY-RUN gpu_video_fpga_top (xc7a35tcpg236-1)" in result.stdout
    assert "Sources: 31" in result.stdout
    assert not out_dir.exists()
